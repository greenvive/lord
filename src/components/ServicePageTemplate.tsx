...
export default function ServicePageTemplate({ content }: { content: ServicePageContent }) {
  const hasCategoryFilter = Boolean(content.caseCategoryTags && content.caseCategoryTags.length > 0);

  const relatedPortfolioItems = hasCategoryFilter
    ? portfolioItems.filter((item) => content.caseCategoryTags!.includes(item.category)).slice(0, 4)
    : content.casePlaceTag
      ? portfolioItems.filter((item) => item.place === content.casePlaceTag).slice(0, 4)
      : portfolioItems.slice(0, 2);

  // "전체 포트폴리오 보기"는 카테고리 태그가 있으면 해당 카테고리로, 없고 장소 태그(casePlaceTag)만 있으면
  // 카테고리는 전체로 두고 장소만 프리셋한다.
  const portfolioHref = hasCategoryFilter
    ? `/portfolio?category=${encodeURIComponent(content.caseCategoryTags!.join(","))}`
    : content.casePlaceTag
      ? `/portfolio?place=${encodeURIComponent(content.casePlaceTag)}`
      : "/portfolio";
      ...
      <section className="px-5 py-16">
        <div className="mx-auto max-w-4xl space-y-16">
          ...
          <div>
            ...
            <Link href={portfolioHref} className="mt-4 inline-block text-sm font-bold text-lord-orange">
              전체 포트폴리오 보기 →
            </Link>
          </div>
